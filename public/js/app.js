...
function handleColorPicker(event) {
  ...
  $('#socialTag1, #socialTag2, path').css({ 'color': color });
  $('path').attr('fill', color);
}

$(function () {
  if (window.page === 'main') {
    $('#logoColor').change(function (e) {
      return handleColorPicker(e);
    });

    $('form').submit(function (e) {
      $('.alert').addClass('hidden');

      var tag1 = $('#socialTag1').val().toLowerCase();
      var tag2 = $('#socialTag2').val().toLowerCase();

      if (tag1 == 'social' || tag2 == 'social') {
        $('#socialTag1, #socialTag2').val('');
        $('.alert').removeClass('hidden');
        return false;
      }

      $('.btn-generate').text('Generating...');
      return true;
    });
  }
})
...
function appendAddItemButton() {
  $('.product-container').map(function (idx, container) {
    var prodType = $(container).data('product');
    var product = window.products[prodType];

    var colorOptions = '';
    Object.keys(product.colors).map(function (key) {
      colorOptions += '<option data-hex=\'' + product.colors[key] + '\' value=\'' + key.toUpperCase() + '\'>' + key.toUpperCase() + '</option>';
    });

    var sizeOptions = '';
    Object.keys(product.sizes).map(function (key) {
...
function handleCartSubmission() {
  $('.proceed-to-checkout').click(function (e) {
    ...
    Object.keys(window.products).map(function (item) {
      var qty = parseInt($('[data-' + item + '-qty]').val());
...
window.evalItemColor = function (item) {
  var colorName = $('[data-' + item + '-color]').val();
  var colorHex = $('[value="' + colorName + '"]').data('hex');
...
window.evalQtyChange = function (item) {
  var qty = parseInt($('[data-' + item + '-qty]').val());
  var costPer = parseFloat(window.products[item].cost);
